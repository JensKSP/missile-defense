...
`missile_defense.runs.archive` has been tested since Task 9 and none of it was reachable from the
...
def test_restoring_over_an_existing_run_is_refused(
    ...
    from missile_defense.runs import archive
    from missile_defense.ui import storage
    from PySide6.QtWidgets import QFileDialog, QMessageBox
...
def test_a_model_can_be_deleted_out_of_the_league(
    ...
    # The view deletes through `missile_defense.runs.paths`, exactly as the trainer does, so the
...
def test_importing_a_name_the_league_already_has_asks_before_replacing(
    ...
    around it — that the answer reaches `missile_defense.runs.league` and produces one entry
