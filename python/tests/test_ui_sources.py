...
M8, risk 2), and they are testable at all only because ``missile_defense.runs.sources`` holds no
...
import pytest
from missile_defense.runs.sources import (
    BASELINE_MEAN_SCORE,
    MAX_RUN_CHOICES,
    NO_CHECKPOINTS,
    RECENT_POINTS,
...
def test_the_log_tail_yields_the_lines_appended_since_the_last_poll(tmp_path: Path) -> None:
    ...
    # itself (missile_defense.runs.runlog), which is what makes an attached run's log pane work.
