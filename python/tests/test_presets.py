...
No Qt in :mod:`missile_defense.runs.presets`, so the whole create/read/update/delete cycle is
...
import pytest
from missile_defense.runs import presets
from missile_defense.runs.presets import PresetError
...
def test_best_splits_its_bigger_batch_into_smaller_pieces_not_larger_ones() -> None:
    ...
    # memory on a 32 GiB card before the first update finished. See missile_defense.runs.footprint.
    good = presets.find("good")
    best = presets.find("best")
