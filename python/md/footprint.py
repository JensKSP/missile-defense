...
* The **rollout buffer** is `envs × steps` samples, preallocated once
  (:class:`md.ppo.Rollout`). It scales with the batch, and it is the term people
  expect.
* The **minibatch** is what the update actually pushes through the network, and
  ...
  (:mod:`md.auxiliary`), so peak memory follows `batch / minibatches` — *not*
...
The constants are **measured**, not derived: two updates of `md.train` on an
