...
So the trainer window has ``md.ui.__main__`` and the training command has this.
...
def split_multiseed(argv: list[str]) -> list[str] | None:
    ...
    it belongs to the child trainers (md.multiseed), and a run that passes the
    literal string down to them — ``-- --multiseed`` — must reach md.train and be
