...
This module deliberately stays one process outside :mod:`md.train`.  Each seed
...
def _print_summary(results: Sequence[SeedResult], winner: SeedResult | None) -> None:
    ...
    if winner is not None:
        ...
        print(
            ...
            f"decisions are final:\n  {sys.executable} -m md.train --load {winner.checkpoint}"
        )
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Python interpreter used for each md.train child.",
    )
    parser.add_argument(
        "--aggregate-only",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="Arguments after -- are passed to every md.train process.",
    )
    return parser
