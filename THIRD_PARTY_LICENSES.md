...
compiled Python extension; the training console (`missile-defense-training`) is
...
what a release actually redistributes, and the console redistributes none of its
Python dependencies — they come from the distribution's own packages, from the
user's `pip`, or from the managed runtime the console installs into the user's
...
## The training console — depended on, never redistributed

None of these is inside any artifact a release publishes. The Debian console
package *depends* on the distribution's copies; the Windows and macOS console
...
know what the console will pull onto their machine before it does.
...
**Why PySide6 being LGPL does not make this project LGPL.** The console imports
...
with Qt itself, and the reason the console is a *separate package* that the
...
bundle PySide6 (a frozen console, say), the full Qt obligation below applies to
it unchanged.
...
console payload, is built with **nanobind** (© 2022 Wenzel Jakob), whose runtime
