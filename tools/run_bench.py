...
``poe profile`` builds the ``profile`` preset instead (per-phase timers on, frame
pointers kept for external samplers) and passes ``--profile``.
...
def main(argv: list[str] | None = None) -> int:
    ...
    preset = "profile" if "--profile" in args else "release"
