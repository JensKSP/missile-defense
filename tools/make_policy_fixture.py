...
through `missile_defense.sim.policy_format`, so this runs on any machine and produces the same
...
def build_entity() -> policy_format.NativePolicy:
    ...
    Built tensor by tensor rather than from
    `missile_defense.training.ppo.EntityPolicy`, so this needs
    no torch and produces the same bytes anywhere — the same reason `build` does.
    `missile_defense.sim.export_policy` pins the numpy reference against torch separately; this
